...
- **`[limits]` DoS 防护**（默认值，可调）：
  ...
  - `connect_timeout = 12s`：所有上游候选共享整体拨号时限
...
- **身份池空时不发起新注册**：保留当前身份，避免在死循环里消耗注册配额

## 6. 已知限制

- **IPv6 依赖 WARP 下发双栈地址**：若当前身份只拿到 IPv4 隧道地址，IPv6 目标会被过滤并回退到 IPv4 候选
